...
const mupdfWorker = {
  ...
  async imageToPdf(imageData: Uint8Array, mimeType: string): Promise<Uint8Array> {
  ...
  async extractPages(pdfData: Uint8Array, pageIndices: number[]): Promise<Uint8Array> {
    const srcDoc = mupdf.Document.openDocument(pdfData, "application/pdf").asPDF();
  ...
  async mergePdfs(pdfBuffers: Uint8Array[]): Promise<Uint8Array> {
    ...
    try {
      for (const data of pdfBuffers) {
        const srcDoc = mupdf.Document.openDocument(data, "application/pdf").asPDF();
        if (srcDoc) {
  ...
  async resizePdf(pdfData: Uint8Array, width: number, height: number): Promise<Uint8Array> {
    const srcDoc = mupdf.Document.openDocument(pdfData, "application/pdf").asPDF();
    if (!srcDoc) throw new Error("Could not open source PDF");
    const outDoc = new mupdf.PDFDocument();
    try {
  ...
  async getFonts(pdfData: Uint8Array) {
    ...
    try {
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        stext.walk({
          onChar(_c: any, _origin: any, font: any) {
