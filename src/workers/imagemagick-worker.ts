...
const imagemagickWorker = {
	async getWritableFormatOptions(): Promise<FormatOption[]> {
		...
		return requestedFormats.filter((candidate) =>
			writableFormats.has(candidate.format),
		);
	},
